...
全体の設計と根拠は `docs/260910_dev_plan.md`（v1.5）にあります。迷ったらそちらが正。
...
**Phase 3**（ホーム画面ウィジェット）は、**Phase 2** より先に行った（D-16）。3-7（1 週間の運用）を残して実装が済んだ。
...
**3-4（置き方のガイド）も済み**: **基本の置き方は、大を 1 つ、ページのいちばん上に**（D-31。ページの部屋が無いうちは、
...
（`WeekRunReport`）。**3-7 のあいだはビルドを替えない**（実機に入れ直さない）。3-8（ページの部屋）は 3-7 のあと（いつ作るかは Q-22）。

**次の山は Phase 2**（プラン v1.4 で詳細化。§9 の 2-A〜2-I。09/28〜11/01）。既定の 5 体を本番の絵にし、利用者が自分の
ChatGPT / Gemini / Claude で作った絵を取り込む「キャラ工房」を Phase 5 から移した。**アプリの中では生成しない**（API を呼ばない。D-32・D-33）。
既定のキャラも取り込んだキャラも、同じ形・同じ整え方・同じ描き方にする（D-34）。整える処理は新しいパッケージ **CTStudio** に
1 つだけ書き、アプリの取り込みと既定のキャラを焼く Mac の道具が同じ関数を呼ぶ（D-35。ウィジェットにはリンクしない）。
背景は自前の処理（透明 → 縁からつながる 1 色の地）を先に、Vision を予備にする（D-36。**Vision はシミュレータで動かない**）。
2-0（ユーザーが iPhone だけで 1 体作ってみる）から始め、未決 Q-18〜Q-22 をそろえる。
キャラを作るプロンプトは `docs/260926_prompt_templates.md`（フェーズ A 試し・B 既定の 5 体・C 本番運用。D-37）。
取り込みの条件の行（全身・同じ大きさ・同じ接地線・広い余白・影なし・文字や枠線なし）と背景の行は、アプリの取り込みが頼るので変えない。
