...
全体の設計と根拠は `docs/260910_dev_plan.md`（v1.6）にあります。迷ったらそちらが正。
...
（`WeekRunReport`）。**3-7 のあいだはビルドを替えない**（実機に入れ直さない）。3-8（ページの部屋）は Phase 2 のあと、予備の 2 週間で作る（Q-22）。
...
2-0（ユーザーが 1 体作ってみる）は済み。Q-18〜Q-22 は推奨どおりに決まった（既定の 5 体は生成 AI で作り直す・取り込みは 10 体まで・
SVG は外部のパッケージ SwiftDraw で絵にする〈D-39。CTStudio にだけ入れ、ウィジェットには入れない〉・引っ越し用の書き出しを入れる・3-8 は Phase 2 のあと）。
キャラを作るプロンプトは `docs/260926_prompt_templates.md`（フェーズ A 試し・B 既定の 5 体・C 本番運用。D-37）。
取り込みの条件の行（全身・同じ大きさ・同じ接地線・姿勢を縦にまとめる・広い余白・影なし・文字や枠線なし）と背景の行は、アプリの取り込みが頼るので変えない。
**2-0 の試しは済み**（ChatGPT のピヨ 3 枚。切り分けは手直しなしで 100%）。まばたき・寝息の 2 コマ目はアプリが作り、割り方は利用者 6 コマ・
既定の 5 体 4 コマ×2（D-38）。生成 AI の透明 PNG は体の中の不透明さが 253〜254 で 255 にならないので、取り込みでそろえる。
足元は輪郭線のいちばん下を枠の 94.7% に置き、倍率は全コマが縦長の枠に収まる値にする（プラン 2-C ⑤）。
v2 の確かめ（A-7、ポーズ 6 コマ）は合格（縮めずに収まり、立ち姿 449 画素）。iPhone の写真に保存しても透明の PNG のまま（1 バイトも変わらない）。
