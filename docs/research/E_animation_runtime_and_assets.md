...
- 分・時の数字と `:` は **送り幅 0 の空グリフ**にして非表示化。秒だけを拾うには GSUB `liga` で「`:` + 十の位 + 一の位」の 3 文字合字を `s00`〜`s59` に置換する（🔷 筆者再構成。Bryce の実装は動画で要確認 [S47]）。`showsHours: false` にして `H:MM:SS` 形式を避ける（✅ パラメータ存在 [S37]）。
