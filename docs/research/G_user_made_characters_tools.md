...
- **形式は未確認**: 写真アプリに PNG のまま（アルファつきで）入るか、「"ファイル"に保存」で PNG か、公式の記載も確かな検証も見つからない。2024 年（DALL·E のころ）は名前が .png で中身が WebP だった報告 🔶[S8]。Web のダウンロードは PNG との記事 🔶[S21]。（→ 2026-09-26 の A-7 で確かめた: iPhone の ChatGPT から写真に保存し AirDrop で送った絵は、Mac で保存した絵と 1 バイトも違わない透明の PNG だった。`docs/260926_prompt_templates.md` §2.2）
...
- 縦横比: 自由。縦横比の選択か、プロンプトで指定 ✅[O1]。API は 1:3〜3:1、辺は 16 の倍数・3840 以下 ✅[O9]。**アプリの出力画素数は未確認**。（→ 2026-09-26 の 2-0 で測った: ChatGPT は約 157 万画素。1:1 は 1254×1254、3:4 は 1086×1448。Web から保存。`docs/260926_prompt_templates.md` §2.1）
...
1. ChatGPT iPhone アプリの「保存」で写真アプリに入る形式（PNG のままか、アルファが残るか）。「"ファイル"に保存」と「コピー」の形式。（→ 2026-09-26 の A-7 で確かめた: iPhone の ChatGPT から写真に保存し AirDrop で送った絵は、Mac で保存した絵と 1 バイトも違わない透明の PNG だった。`docs/260926_prompt_templates.md` §2.2）
2. ChatGPT アプリの出力画素数（縦横比ごと、プランの差）。（→ 2026-09-26 の 2-0 で測った: ChatGPT は約 157 万画素。1:1 は 1254×1254、3:4 は 1086×1448。Web から保存。`docs/260926_prompt_templates.md` §2.1）
